...
* 根据恩山论坛dominic的帖子：[开启RTL8125B的多队列支持和关闭ASPM](https://www.right.com.cn/forum/thread-7446026-1-1.html)

* 软路由是r8125b网卡的均支持(硬件差异比较大，不保证百分百)
...
#### 2.将解压出来的deb格式的驱动放到pve系统的root目录下，然后终端执行：
...
* 继续终端执行：`uname -r`或者当前pve系统的内核版本，并复制下来：
...
#### 3.重启pve后，进入pve控制台，查找r8125b网卡的名称；

每个人的机器，显示的名称的不一样，自己的网卡具体是啥名称，自行判断！

![jpg](./pic/42.jpg)

#### 4.然后终端执行：`ethtool -i enp4s0` (请将enp4s0换成你自己的网卡名称)，可以看到r8125b的驱动已经替换成功。

![jpg](./pic/43.jpg)


#### 5.最后终端执行 `grep -E enp* /proc/interrupts` 就可以看到是多个队列了。
